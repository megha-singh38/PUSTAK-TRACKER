...
from app.models import User, Book, Transaction, Category, Reservation
...
if __name__ == '__main__':
    ...
    if os.getenv('FLASK_ENV') != 'production':
        ...
        print("\n" + "="*60)
        print("Starting Pustak Tracker - Librarian Web")
        print("="*60)
        print(f"\n>> Web Access:")
        print(f"   https://{local_ip}:{port}")
        print(f"\n>> Barcode Scanner URLs:")
        print(f"   Scanner: https://{local_ip}:{port}/mobile_scanner.html")
        print(f"\n>> Default Librarian Credentials:")
        print(f"   Email: librarian@pustak.com")
    ...
    if os.path.exists('cert.pem') and os.path.exists('key.pem'):
        ...
        port = int(os.getenv('PORT', 5000))
        print("\n" + "="*60)
        print("WARNING: SSL certificates not found. Running HTTP only.")
        print("Run: openssl req -x509 -newkey rsa:4096 -keyout key.pem -out cert.pem -days 365 -nodes")
        print(f"\n>> Web Access: http://{local_ip}:{port}")
        print(f"\n>> Barcode Scanner URLs:")
        print(f"   Scanner 2: http://{local_ip}:{port}/mobile_scanner.html")
        print("="*60 + "\n")
        app.run(
            host='0.0.0.0',
            port=port,
